const Api = (() => {
  ...
  async function finmind(dataset, params = {}){
    ...
    const url = buildUrl(cfg.finmindBase, { dataset, token, ...params });
  ...
  async function getDailyPrice(code, startDate, endDate){
    const data = await finmind('TaiwanStockPrice', { data_id: code, start_date: startDate, end_date: endDate });
    const rows = Array.isArray(data.data) ? data.data : [];
    return rows.map(r => ({
      date: r.date,
  ...
  async function getFugleQuote(code){
    ...
    const url = `${cfg.fugleBase}/intraday/quote/${encodeURIComponent(code)}`;
    return safeFetch(url, { headers: { 'X-API-KEY': key }});
  }
  ...
  async function askOpenAI(prompt){
    ...
    const body = {
      ...
      messages: [
        { role: 'system', content: '你是台股量化交易助理。回答要精簡、保守、重視風險，不保證獲利。' },
    ...
    if(!res.ok){
      const text = await res.text().catch(()=> '');
      throw new Error(`OpenAI HTTP ${res.status} ${text.slice(0,120)}`);
    }
  ...
  return { getSettings, saveSettings, getStockInfo, getDailyPrice, getFugleQuote, askOpenAI, test };
})();
